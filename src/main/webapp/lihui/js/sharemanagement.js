...
//	var datagrid = $('#mytable').datagrid();
	var row = $('#sharetable').datagrid('getSelected');
        	    if (row){
...
$(function () {
    ...
    datagrid = $("#sharetable").datagrid({
        url: '/Share_system/share/queryAllShareByAdmin.do', //请求的数据源
        method: 'get',
...
//           editor: { type: 'validatebox', options: { required: true} }
        ...
        {field: 'subject', title: '主题', width: 180, sortable: true
//           editor: { type: 'validatebox', options: { required: true} }
        },
         { field: 'content', title: '内容', width: 400, sortable: true
//             editor: { type: 'validatebox', options: { required: true} }
         },
         { field: 'attachements', title: '附件', width: 300, sortable: true//附件不允许编辑
//             editor: { type: 'validatebox', options: { required: true} }
         },
...
//              editor: { type: 'validatebox', options: { required: true} }
         ...
         { field: 'shareDate', title: '分享日期', width: 100
//               editor: { type: 'validatebox', options: { required: true} }
           },
           { field: 'studentNum', title: '听课人数', width: 80
//               editor: { type: 'validatebox', options: { required: true} }
           },
           { field: 'grade_num', title: '评分人数', width: 80//评分人数不允许编辑
//               editor: { type: 'validatebox', options: { required: true} }
           },
           { field: 'adGrade', title: '管理员评分', width: 90,
               editor: { type: 'validatebox', options: { required: true} }
           },
           { field: 'grade', title: '平均得分', width: 80//
//               editor: { type: 'validatebox', options: { required: true} }
           },
           { field: 'lastModifyTime', title: '最后更新时间', width: 150//
//               editor: { type: 'validatebox', options: { required: true} }
           }
           ]],
...
//        }, '-',
        ...
        onAfterEdit: function (rowIndex, rowData, changes) {
            ...
            $.ajax({  
	               type: "post",  
	                url: "/Share_system/share/addOrUpdateAdminGrade.do",  
	                data: 
	  			    {//管理员只允许更新以下字段：管理员评分，其他字段不允许更新
	  			    	"s_id":rowData.shareId,
//	  			    	"type":rowData.type,
//	  			    	"subject":rowData.subject,
//	  			    	"content":rowData.content,
//	  			    	"stu_num":rowData.studentNum,
	  			    	"ad_grade":rowData.adGrade
